...
export default function SummaryScreen() {
  ...
  // Fetch budgets and expenses for selected period
  useEffect(() => {
  ...
  const totalRestante = useMemo(
    () =>
      presupuestos.reduce((s, p) => {
        const gastCat =
          gastos.filter(g => g.categoria === p.categoria).reduce((a, g) => a + g.monto, 0);
  ...
  const onPressCard = (p: any) =>
    navigation.navigate('BudgetDetailScreen', { budget: p });
    ...
    >
      ...
      <View style={styles.totalsContainer}>
        <Text style={styles.totalText}>
          Gastado: ${formatNumber(totalGastado)}
        </Text>
        <Text style={styles.totalText}>
          Restante: ${formatNumber(totalRestante)}
        </Text>
      </View>
      ...
      {loadingMedios ? (
        ...
        <PieChart
          data={Object.entries(gastosPorMedio).map(([name, amt], idx) => ({
            name,
            amount: amt,
            color: styles.colors[idx % styles.colors.length],
      ...
      <PieChart
        data={Object.entries(gastosPorCategoria).map(([name, amt], idx) => ({
          name,
          amount: amt,
          color: styles.colors[idx % styles.colors.length],
          legendFontColor: '#7F7F7F',
      ...
      {presupuestos.map(p => {
        ...
        return (
          ...
          >
            ...
            <Text style={{ color: rem < 0 ? 'red' : 'green' }}>
              Restante: ${formatNumber(rem > 0 ? rem : 0)}
            </Text>
          </TouchableOpacity>
        );
      })}
